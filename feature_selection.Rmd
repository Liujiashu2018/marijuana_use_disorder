...
#NSDUH_2021$eduskip <- as.factor(NSDUH_2021$eduskip)
...
trainx <- as.matrix(data.frame(trainfactors, train_data$bmi, train_data$mentalhealth))
```
...
testx <- as.matrix(data.frame(testfactors, test_data$bmi, test_data$mentalhealth))
```

```{r}
CVlasso <- cv.glmnet(trainx, 
                     train_data$SUD_MJ, 
                     type.measure = "class", 
                     family = "binomial", 
                     alpha = 1, 
...
```
```{r}
ypred <- predict(CVlasso, s = CVlasso$lambda.min, newx = trainx, type = "class")
t1 <- table(ypred, train_data$SUD_MJ)
t1
confusionMatrix(t1)
```
