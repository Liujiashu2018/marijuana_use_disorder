...
NSDUH_2021_full <- PUF2021_100622 %>%
  mutate(SUD_MJ = rowSums(select(., udmjlrgamts, udmjnotstop, udmjtimeget, 
                                 ...
                                 udmjwdshake) %>% 
                            mutate_all(~if_else(. %in% c(83, 85, 91, 94, 97, 98, 99), NA_integer_, .))) >= 2, 
         SUD_MJ = if_else(is.na(SUD_MJ), 0, SUD_MJ))
```
...
NSDUH_2021 <- NSDUH_2021_full %>%
  select(age, sex, race, health, marital, degree, eduskip, employ, family, kid, 
         ...
         pregnant, bmi, schfeel, parhw, mentalhealth)
...
```{r}
write_csv(NSDUH_2021, "/Users/jiashuliu/Desktop/ML&2004/Messy Data/Final_Project/Messy_Data_Final/NSDUH_2021.csv")
```
